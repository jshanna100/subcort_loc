...
fig_dir = join(mem_dir, "figures", "restricted")
data_dir = join(root_dir, mem_dir, "02_MemTask")


signal_df = pd.read_pickle(join(data_dir, "signal.pickle"))
cnx_df = pd.read_pickle(join(data_dir, "cnx.pickle"))
...
fig, axes = plt.subplots(4, 3, figsize=(38.4, 21.6))
idx = 0
...
for stim in ["active", "sham"]:
    ...
    for pp in ["pre", "post"]:
        ...
        sns.heatmap(data=dpli, cmap="seismic", vmin=min_dpli, vmax=max_dpli,
                    ax=axes[idx, 2], center=.5, annot=True, fmt=".2f")
        idx += 1
